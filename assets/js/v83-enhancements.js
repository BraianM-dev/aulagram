...
async function ag83ToggleActive(){const box=adminActiveInput();if(!box)return;try{await g.rpc('adminUpdateSettings',{active:!box.checked},g.state.admin);g.state.bootAt=0;g.toast(!box.checked?'AulaGram activada':'AulaGram pausada');await g.renderAdmin()}catch(e){g.toast(g.niceErr(e))}}
async function ag83ExportMessages(){let w=null;try{w=window.open('about:blank','_blank');if(w)w.document.write('<p style="font-family:Arial;padding:20px">Preparando exportación de mensajes…</p>');const r=await g.rpc('adminExportMessages',g.state.admin);if(!r||!r.ok)throw new Error((r&&r.message)||'No se pudo exportar');g.toast('Exportación creada: '+r.csvName,5000);if(w)w.location=r.csvUrl;else location.href=r.csvUrl}catch(e){if(w)w.close();g.toast(g.niceErr(e),5000)}}
g.ag83ToggleActive=ag83ToggleActive;g.ag83ExportMessages=ag83ExportMessages;
function enhanceAdmin(){const hero=Array.from(document.querySelectorAll('.hero')).find(x=>x.querySelector('h1')?.textContent.includes('Panel docente'));if(!hero||hero.dataset.v83)return;hero.dataset.v83='1';const active=!!adminActiveInput()?.checked;const box=document.createElement('div');box.className='network-status '+(active?'open':'closed');box.innerHTML=`<div><b>${active?'🟢 AulaGram está ACTIVA':'🔒 AulaGram está PAUSADA'}</b><span>${active?'Los estudiantes pueden ingresar según los controles habilitados.':'Los datos siguen guardados. Los estudiantes verán la red pausada.'}</span></div><div class="row"><button class="btn ${active?'danger':'primary'}" onclick="ag83ToggleActive()">${active?'⏸ Pausar AulaGram':'▶ Activar AulaGram'}</button><button class="btn primary" onclick="ag83ExportMessages()">⬇ Exportar mensajes CSV</button></div>`;hero.appendChild(box)}
